      ...
      <div className="bg-white">
        {items.map((item, index) => {
          ...
          return (
            <button
              key={item.id}
              onClick={() => item.type === 'toggle' && toggleValue(item.id)}
...
export default function SettingsPage() {
  return (
    <div className="h-full flex flex-col bg-gray-50">
      ...
      {/* Profile Card */}
      <div className="mx-4 mt-4 p-4 bg-white rounded-2xl shadow-sm">
      ...
      <div className="flex-1 overflow-y-auto pb-20 mt-4">
        ...
        <SettingSection title="Preferences" items={preferenceSettings} />
        <SettingSection title="Other" items={otherSettings} />
      </div>
    </div>
  );
}
