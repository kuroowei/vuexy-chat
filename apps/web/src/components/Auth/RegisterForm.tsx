...
import { Eye, EyeOff, Mail, Lock, User, ArrowRight, CheckCircle2, Camera, X, Phone } from 'lucide-react';
...
export default function RegisterForm() {
  ...
  const isValidPhone = (value: string) => {
    // Accepts common Nigerian formats: 08031234567, 8031234567, 2348031234567, +2348031234567
        ...
        <>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input type="tel" required value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="08031234567"
                className="w-full pl-10 pr-4 py-2.5 bg-white border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all" />
            </div>
            {phone && !isValidPhone(phone) && (
              <p className="text-xs text-red-500 mt-1">Please enter a valid Nigerian phone number</p>
            )}
          </div>
          ...
          <button type="button" onClick={() => setStep(2)} disabled={!name || !email || !phone || !isValidPhone(phone)}
